...
```mermaid
graph TD
UC["Use Cases"] --> Scenario["Scenario-Based Model (who does what, 
when)"]
UC --> Class["Class-Based Model (what data / entities exist)"]
UC --> Functional["Functional Model (how data flows and transform
s)"]
UC --> Behavioral["Behavioral Model (what states an entity can be i
n)"]
```--
...
Validate -->|"access + refresh tokens"| Visitor
```--
## Behavioral Model
### Post State Diagram
```mermaid
